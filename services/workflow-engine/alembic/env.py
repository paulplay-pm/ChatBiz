"""Alembic environment for the chatbiz-workflow-engine service.
...
reuse the same ``Base.metadata`` declared in ``app.models.workflow`` for
...
# Ensure the service root is on sys.path so ``from app.models.workflow``
...
from app.config import get_settings  # noqa: E402  (sys.path tweak above)
from app.models import Base  # noqa: E402  (sys.path tweak above)
...
def _resolve_database_url() -> str:
    ...
    if not url:
        # Fall back to the runtime config — keeps a one-line local dev
        # path (``alembic upgrade head``) working when the env var is
        # unset but the service's .env file is present.
...
def _do_run_migrations(connection: Connection) -> None:
    ...
    context.configure(
        ...
        # ORM models set Python-level default values that mirror the
        # migration DDL, and enabling server-default comparison produces
        # noisy false positives on every autogenerate run.
        compare_type=True,
    )
    with context.begin_transaction():
        context.run_migrations()
