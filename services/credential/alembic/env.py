"""Alembic environment for the credential-management service.

Async template: the connection is opened with ``create_async_engine`` and
then handed to Alembic via ``connection.run_sync(...)``. This lets us reuse
the same ``Base.metadata`` declared in ``app.models`` for autogenerate
without paying the cost of an extra event-loop gymnastics layer.
