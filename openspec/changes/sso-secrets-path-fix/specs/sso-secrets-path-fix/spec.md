...
- **THEN** the output MUST be `(healthy)` (proving the 3-gate `depends_on: sso + workflow-engine + mcp: service_healthy` chain in dev compose's `chatbiz-web` block is now satisfied)
