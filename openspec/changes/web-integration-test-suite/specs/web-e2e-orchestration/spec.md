# web-e2e-orchestration

**Frontend Scope: 含前端**（`web/canvas/e2e/integration/` + `web/admin/e2e/integration/` + `web/nginx.conf` 增量 + `web/Dockerfile` 集成测试 target）

**Backend Scope: 含后端**（`services/audit-and-isolation/app/api/chat.py` 旁路 + `infrastructure/docker-compose-test.yml`）

**Impact**（被谁消费）：
- 被 `canvas-api-integration` capability 消费（共享 compose 启动矩阵）
- 被 `admin-health-integration` capability 消费（共享 nginx `/healthz` 代理）
- 被 MVP 阶段的回归测试消费（`docs/prd.md` §4.2 paul 财务月报 workflow）
- 后续 change `gateway-pii-e2e` / `manual-approval-resume` / `plugin-degradation` 复用本基础设施覆盖 4 critical path 的 ②③④
