# tailwind-theme-prototype-sync

**Frontend Scope: 含前端**（Tailwind CSS 3.4 配置 + prototype.html 色板映射）

**Impact**（被谁消费）：
- 被本 change 其他 capability（side-nav-shell / route-skeleton / placeholder-view）作为基础样式
- 被 `mcp-server-management-ui` task 7.x 复用（卡片网格 / 状态徽章 / 配置弹窗直接用 `ink-*` `brand-*` 类）
