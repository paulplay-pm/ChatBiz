...
The system MUST have `apps/admin-web/playwright.config.ts` with:
- `testDir: './e2e'`
- `timeout: 30_000`
- `fullyParallel: true`
...
The file `apps/admin-web/e2e/admin-web-bootstrap.spec.ts` MUST contain exactly 1 test scenario: "Open /mcp-tools and verify SideNav + Placeholder". The test MUST:
...
The system MUST have `apps/admin-web/vitest.config.ts` with:
...
The file `apps/admin-web/tests/unit/AppShell.test.tsx` MUST contain 1 test:
