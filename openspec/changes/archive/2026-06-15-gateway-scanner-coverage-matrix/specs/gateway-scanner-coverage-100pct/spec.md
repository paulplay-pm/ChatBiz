<!--
Delta spec for capability `gateway-scanner-coverage-100pct`.
...
Capability 范围：services/gateway-scanner/gateway_scanner/ 下 2 个目标模块
  * `scanner.py` — AST walker + Violation / ScannerConfig / load_config / scan_path
  * `__main__.py` — click CLI 入口

源：openspec/changes/gateway-scanner-coverage-matrix/{brainstorm,proposal,design}.md
触发源（retrospective §6.4 第 2 条）：
