# infra-compose-naming

**Frontend Scope: N/A — 纯基础设施层（infrastructure/docker-compose.yml service key 命名），无 UI / 业务 / 协议场景**

**Backend Scope: 含后端**（`infrastructure/docker-compose.yml` 改 ~10 处 service key / depends_on 引用）

**Impact**（被谁消费）：
- 被 `sso-real-impl` change 消费（V6a T5.3-5.5 compose up + 5 路径 curl 验证自动解锁）
- 被任何干净 dev 机 / CI runner / ops 人员消费（`docker compose -f docker-compose-dev.yml config` 不再 v5.0.2 strict FAIL）
- 后续 change 接入 CI / 运维依赖 dev compose 干净启动
