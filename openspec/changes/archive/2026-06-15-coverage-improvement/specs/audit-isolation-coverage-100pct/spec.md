<!--
Delta spec for capability `audit-isolation-coverage-100pct`.

本 change 是 ADDED-only（新建 capability，不修改任何既有
capability 的 REQUIREMENTS）。apply 完成后，本檔被合并到
`openspec/specs/audit-isolation-coverage-100pct/spec.md`。

Capability 范围：services/audit-and-isolation/app/ 下 3 个目标模块
  * `app/jobs/archive_audit.py`
  * `app/llm/client.py` 的 `compute_idempotency_key` 入口
  * `app/routing/table.py`

源：openspec/changes/coverage-improvement/{brainstorm,proposal,design}.md
触发源（retrospective §6.4 第 1 条）：
