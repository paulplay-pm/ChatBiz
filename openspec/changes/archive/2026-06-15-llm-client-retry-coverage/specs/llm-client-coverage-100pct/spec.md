<!--
Delta spec for capability `llm-client-coverage-100pct`.

本 change 是 ADDED-only。apply 完成后被合并到
`openspec/specs/llm-client-coverage-100pct/spec.md`。

Capability 范围：services/audit-and-isolation/app/llm/client.py
  * `get_client` lazy init (line 74-80)
  * `retry_with_redis` body (line 104-120) — 5xx retry +
    connection-interrupted exception retry
  * `_is_ha_failover` JSON parse error path (line 214-215)
  * `reset_client_for_tests` (line 334)

源：openspec/changes/llm-client-retry-coverage/{brainstorm,proposal,design}.md
触发源：
  openspec/changes/archive/2026-06-15-coverage-improvement/retrospective.md §4.2
-->
