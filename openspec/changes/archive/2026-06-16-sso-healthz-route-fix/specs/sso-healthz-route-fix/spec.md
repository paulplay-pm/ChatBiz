...
#### Scenario: Web SSO end-to-end 端点 200 (or surfaces next pre-existing bug)
- **WHEN** the same developer runs `curl -fsS http://localhost:5173/api/auth/sso/jwks.json | head -c 200`
- **THEN** the command MUST exit 0 and the output MUST contain a JSON body starting with `{"keys":` (proving the Web SSO end-to-end path is fully wired: nginx → chatbiz-sso via the upstream proxy) OR the output MUST be a 502 Bad Gateway caused by the next pre-existing bug — `uvicorn --reload` (set in dev compose's sso `command:` for live-reload) interferes with nginx's keepalive connection pool, causing "upstream prematurely closed connection" on the proxy. In the latter case, `docker exec chatbiz-sso-1 python -c "urllib.urlopen('http://127.0.0.1:8007/api/v1/auth/sso/jwks.json').status"` MUST still return `200`, proving the sso FastAPI service itself is healthy and the 502 is purely a dev compose + uvicorn --reload interaction (out of this change's scope).
