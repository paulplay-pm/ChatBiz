...
## Purpose
定义 workflow-engine 的 PostgreSQL 持久化层:5 张业务表 + LangGraph 官方 checkpoints 表的 schema、约束、索引、回滚策略。本 capability 仅 PG,Redis canvas 状态留给 `implement-canvas-ui` change(eng-review Quality #2 拆分)。
## Requirements
## ADDED Requirements
