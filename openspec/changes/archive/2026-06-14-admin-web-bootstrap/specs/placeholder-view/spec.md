...
**Frontend Scope: 含前端**（未实现 menu item 的占位视图）

**Impact**（被谁消费）：
- 被 route-skeleton 消费（14 个路由全部指向 PlaceholderView）
- 未来被业务 change 替换（`mcp-server-management-ui` task 7.3 把 `/mcp-tools` 换成 `McpToolsView`）
