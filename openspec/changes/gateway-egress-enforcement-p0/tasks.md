...
- [x] 4.4 实现冷查询端点 `services/audit-and-isolation/app/api/audit_archive.py` `GET /v1/audit/archive?from=...&to=...`:从 MinIO 拉 parquet,异步返回,响应头 `X-Audit-Source: cold`,4.4 验证:`tests/integration/test_audit_archive_endpoint.py` 验证查询 + 响应头 + MinIO 失败 503 ✅ 2026-06-14 完成(10/10 PASS,jsonl 跟 4.3 一致)
