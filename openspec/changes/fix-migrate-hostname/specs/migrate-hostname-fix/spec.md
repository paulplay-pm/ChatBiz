...
- **WHEN** a developer runs `grep -c "postgres:5432" infrastructure/docker-compose.yml` after the change
...
- **WHEN** the same developer runs `grep -c "postgres:5432" infrastructure/docker-compose-test.yml` and `grep -c "postgres:5432" infrastructure/docker-compose-e2e-ha.yml`
