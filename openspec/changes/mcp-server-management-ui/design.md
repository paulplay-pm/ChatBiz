...
`mcp-server-management-ui` 落地**管理面**——让 admin / paul / leo / anny 在 admin 里看见/增删/启停这些 MCP server，**不**改 MCP 协议层、**不**改单 server 实现。
...
**Goals：**
- 让 admin 在 admin 看到当前已注册的 MCP server 卡片（图标、名称、副标题、连接状态、Transport、工具列表、配置/断开按钮），与 prototype.html 视觉一致
...
**Context**：原型是 admin 一张视图，后端需要 1 组 REST 端点。
...
- C. 把 REST 端点直接挂到 admin 的 BFF 进程：拒绝，因为 admin 是 Vite/React，没有 Python 进程；要么引 Python BFF（= 选 B），要么直接调 audit-and-isolation（强耦合，破坏 mcp 单 service 自洽）。
...
**选项**：
- A. **（已选）** 继续 Starlette，用 `Route` + `Request` + `JSONResponse`。admin 调用方代码量 0 多余依赖。
...
**选项**：
- A. **（已选）** admin 已有 React + Hooks 栈，引入 `swr` 做 5s 间隔轮询 `GET /v1/mcp/servers`。MVP 简单，CDN 友好。
- B. WebSocket：拒绝——admin 没有 WS gateway，5s 轮询延迟对 admin 视角可接受。
- C. SSE：拒绝，admin 不引 EventSource 抽象。
...
- **[Risk] admin 不在本 change scope（前端仓库），但需要改 `SideNav.tsx` + `router/index.tsx`** → Mitigation：开**两个** change 入口（`mcp-server-management-ui` 主 + `admin-nav-update` 微），或者本 change 在 `web/admin/` 提交，依赖前置：`admin` 仓库已 git submodule 或 monorepo。**当前仓库结构**：`web/admin/` 目录在 `admin-bootstrap` 前**尚未就位**（CLAUDE.md 提示 admin 尚未搭建）。**决策点**：本 change 在 tasks.md 写"前置：确认 `web/admin/` 已 init"，否则 spec 不 apply。
...
3. admin 部署新版本（含 `/mcp-tools` 路由）
4. 验证：`curl http://chatbiz-mcp:8004/v1/mcp/servers` 返 `[]`，admin 看到空状态
...
- OQ1：admin 是 monorepo 还是 submodule？现状：仓库根当时**没有** `web/admin/` 目录（CLAUDE.md 写"0 行源代码"），prototype.html 是 `docs/` 下的 HTML。前端代码仓库路径未定。**行动**：本 change 在 `tasks.md` 写"前置任务 = 等 admin 仓库就位"，否则 spec 标 `[BLOCKED]`。
