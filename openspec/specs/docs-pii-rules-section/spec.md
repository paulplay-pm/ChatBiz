# docs-pii-rules-section Specification
...
MUST `docs/architecture.md` 必须在 §4.3 末尾新增 §4.3.Y 段落,内容包含:1. PII 6 类正则:中国大陆身份证 / 手机号 / 银行卡 / 邮箱 / 统一社会信用代码 / 营收金额
...
MUST `CLAUDE.md` 必须在 §4.3.Y 段落新增前 surface 此事,加一行 `[FUTURE-IMPLEMENTATION] docs/architecture.md §4.3.Y PII 规则集段落即将在 gateway-egress-enforcement-p0 apply 阶段补,引用 `services/audit-and-isolation/app/pii/` 作为权威实现`。
...
MUST `docs/architecture.md` §4.3.Y 段落补全必须在 `services/audit-and-isolation/app/pii/` 现有实现**稳定**(无 breaking change 计划)后落地,避免文档与代码脱节。
